...
class SalesOrdersExport implements FromCollection, WithHeadings, WithMapping, WithStyles, ShouldAutoSize
{
    ...
    {
        return SalesOrder::with('creator', 'requestOrder')->latest()->get();
    }
    ...
    public function map($order): array
    {
        ...
        return [
            $this->rowNumber,
            $order->sales_number,
            $order->requestOrder->request_number ?? 'Manual',
            $order->customer_name,
            strtoupper($order->status),
            '$ ' . number_format($order->total_amount, 2),
            $order->creator->name ?? 'N/A',
            $order->confirmed_at ? $order->confirmed_at->format('Y-m-d H:i') : 'N/A',
