            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-8 text-gray-900 dark:text-gray-100">
                    <div class="flex items-center justify-between mb-8">
                        <div>
                            <h1 class="text-3xl font-extrabold tracking-tight">Create New Product</h1>
                            <p class="mt-2 text-sm text-gray-600 dark:text-gray-400">Fill in the details below to add a new item to your inventory.</p>
                        </div>
                    ...
                    <form action="{{ route('products.store') }}" method="POST" class="space-y-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <input type="text" name="name" id="name" value="{{ old('name') }}" class="block w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 shadow-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition sm:text-sm p-3" placeholder="e.g. Premium Wireless Headphones" required>
                                @error('name')
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <div class="relative rounded-lg shadow-sm">
                                    ...
                                    <input type="number" name="price" id="price" value="{{ old('price') }}" step="0.01" class="block w-full pl-7 rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition sm:text-sm p-3" placeholder="0.00" required>
                                </div>
                                @error('price')
                                    <p class="mt-2 text-sm text-rose-600 font-medium">{{ $message }}</p>
                                @enderror
                            ...
                            <div>
                                ...
                                <input type="number" name="stock" id="stock" value="{{ old('stock') }}" class="block w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition sm:text-sm p-3" placeholder="Qty" required>
                                @error('stock')
                                    <p class="mt-2 text-sm text-rose-600 font-medium">{{ $message }}</p>
                                @enderror
                            </div>
