            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-xl p-8 border border-gray-100 dark:border-gray-700">
                <form action="{{ route('warehouses.update', $warehouse) }}" method="POST" class="space-y-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="name" id="name" value="{{ old('name', $warehouse->name) }}" class="block w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 shadow-sm focus:ring-indigo-500 focus:border-indigo-500 p-3" required>
                        @error('name')
                    ...
                    <div>
                        ...
                        <input type="text" name="location" id="location" value="{{ old('location', $warehouse->location) }}" class="block w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 shadow-sm focus:ring-indigo-500 focus:border-indigo-500 p-3">
                        @error('location')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="flex items-center">
                        ...
                        <input type="checkbox" name="is_active" id="is_active" value="1" {{ old('is_active', $warehouse->is_active) ? 'checked' : '' }} class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500">
